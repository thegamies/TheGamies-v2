...
| 2026-08-16 | List card interaction | Poster/Grid: hold (~280ms) to drag (scroll locked during hold); tap opens an **external Remove popover** (document-absolute so zoom stays glued; ink-contrast border; design-system `Button` **danger**) that points at the card; tap elsewhere dismisses. Poster drag overlay uses on-screen size so the card tracks the finger under canvas scale. **List:** text **Remove** beside the title; hold-to-reorder. |
