...
| 2026-08-17 | Community invites | Communities are **private**. `/communities` lists **memberships only**. Join requires a current **invite code** (`/communities/join/[code]`). Admins always get **Copy invite** in the community header; they rotate the code and may enable **open invites** (members copy from the header) under Settings → Invite. |
