...
| Auth | Neon Auth (Managed Better Auth); profiles in app `profiles` table. Account deletion closes the auth user via SDK `deleteUser`, the Neon Auth Users API, and `neon_auth` SQL (SDK delete is often a no-op). |
