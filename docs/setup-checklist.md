...
- [ ] Add trusted domains in Neon Auth for **production** hosts (Console → Auth → Configuration → Domains). Staging CI registers `STAGING_CF_APP_URL` on Neon branch `develop`. `localhost` ports are pre-approved; **LAN IPs are not** — for phone/device testing add the URL you open on the phone (e.g. `http://192.168.1.123:3000`). `next.config.ts` allowlists this machine’s current LAN IPs for `/_next` assets; restart `next dev` after a network change. Extra hostnames go in Doppler `ALLOWED_DEV_ORIGINS`.
- [ ] Google sign-in: create a Google Cloud **Web** OAuth client. Authorized JavaScript origins = app origins (`http://localhost:3000`, staging Worker, `https://thegamies.gg`) plus each lasting Neon **Auth host** (scheme + host only). Redirect URIs = `{Auth URL}/callback/google` per lasting branch (local, `develop`, production). Paste Client ID/secret into that branch’s Auth → Google (replace Shared keys). Consent screen app name **The Gamies**; Testing mode needs your Google account as a test user.
