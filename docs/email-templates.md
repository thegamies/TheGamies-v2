...
Local `dev_personal` Neon branch: run `scripts/ci/register-neon-auth-email-webhook.sh <branch_id> <https-origin>` after `pnpm preview:cf`, or set the webhook in Console. `next dev` on Node cannot send via the `EMAIL` binding, so **local laptop sign-up only** skips confirm-email (marks the Neon Auth user verified in `neon_auth`). If Neon **Require email verification** / confirm-email is off, the app also skips that screen (unverified users already have a session). Pull-request previews, develop/staging, and production still show it when Neon requires confirmation.
