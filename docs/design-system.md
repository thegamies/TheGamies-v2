...
- Game detail (`/games/[slug]`): 240px cover on the left; library controls sit in a `--panel` surface flush under the cover (`border-line`). Title, four-line description, **Broadcast compact**, category #1s, credits, **Videos**, **Screenshots**, then **Images** stay **to the right of the cover**. Status picker is `Dialog`. Omit empty kinds; hide Image tabs when only one type remains.
...
- Homepage **Big Picture** hero: cover marquees sit under a transparent site nav; tagline + Browse games / Communities overlay the bottom of the wall. Covers are decorative (not links). Covers from IGDB catalog **popularity** for the current and previous year (same sort as `/games`). Soft side/bottom fade into paper (no hard rule); pauses under `prefers-reduced-motion`. A short gap sits under the wall, then the first landing Top 5 (GOTY, then Top Categories, year on both headings). Guests then get **What is The Gamies?**, a **Game of the Year** chapter (create list / view results, still beside copy, first still on the left then sides swap), then **Communities** with a still on the left. Copy sits against the still; titles sit close above each split. No rules between those pitch chapters. Pitch chapters stay denser than a magazine feature well. Everyone then gets capped Trending / Upcoming, then later landing years. Plate covers are decorative stills, not a second linked board.
