...
- **Eligibility** per category: current year, any year released, upcoming. **Current year** is that list year only. **Any year released** is already out and not later than the list year (Best Ongoing Game, Best Game You Finally Played). **Upcoming** is later years only — not the list year, even if still unreleased. Remake/DLC categories allow edition/version titles. Search and save hide packs, DLC add-ons, bundles, mods, episodes, seasons, ports, forks, and updates; expansions stay. **Best Expansion / DLC** also allows DLC add-ons.
- **Best Ongoing Game** and **Most Anticipated Game** are only offered on the **current and previous** calendar-year lists and events. Existing picks on older years stay; new adds are blocked. **Best Game You Finally Played** stays on every year.
...
- `/` — Cover wall with tagline plus **Browse games** / **Communities**. The **first** landing Top 5 year sits under the hero, titled **{year} Game of the Year**, then **{year} Top Categories** (guests and signed-in). Guests then get **What is The Gamies?**, a **Game of the Year** chapter (create list + view results, still beside copy), a **Communities** chapter, then capped **Trending** and **Upcoming**, then later landing years with the same **{year} Game of the Year** then **{year} Top Categories** headings. **Signed-in** skip the pitch chapters. The newest landing year is always first. Year and **{year} Top Categories** are display links into the year board (hover accent). Bordered **Full Standings** / **Create list** / **My list** sit on the year row; **See All** / **Make picks** / **My picks** sit on Top Categories. Years with no category highlights still show Top Categories (underlined) and a taller centered empty: “Not enough votes yet” plus a bordered **Add categories to your list**.
- `/about`, `/about/lists`, `/about/communities`, `/about/library`, `/about/people`, `/about/pickem`, `/rankings` — original desk copy (identity, lists, communities, library, people, Pick’em, scoring method). Not catalog pages.
- `/games/[slug]` — 240px cover on the left; title, description, **On The Gamies** context (rank + capped public lists) when the title has site value, site GOTY **rank** per public year (Broadcast compact), **site category #1s**, and credits to the right. Description clamps to four lines with Show more / Show less. Rank is always public. Votes, points, and the list-position chart follow the reveal gate. Query is this game’s score rows plus a higher-score count for place — not the full year board. Editions with no scores inherit the parent title’s standings. Suppressed years are omitted. **Indexed and eligible for ads only when the page has original site value** (public GOTY presence, category #1, or a public owned list).
- `/game-of-the-year` — all public years (enough GOTY lists **or** at least one public category): Game of the Year top 5 plus the five most-voted category **#1s**. Header **All** stays on the right (including small screens), aligned with the info control, and opens a year popover into `/game-of-the-year/[year]`. Year is a display link into the year board; **{year} Top Categories** and bordered **See All** open `/game-of-the-year/[year]/categories`. Bordered **Full Standings** / **Create list** and **Make picks** / **My picks** stay on the right. Empty category years keep the Top Categories rule and a centered empty.
- `/admin/seed` — Top 5 for every year with live scores
- `/game-of-the-year/[year]` — GOTY cover-card grid paginated **50 per page** (`?page=2`); secondary **Game of the Year** · **Categories** (`/game-of-the-year/[year]/categories`); **Create list** / **My list** (GOTY view) or **Make picks** / **My picks** (Categories view) sit next to the list total, as a text link into the GOTY creator; category chapters filtered by award group via a single dropdown (`?group=genre`) and searchable by name; categories ordered by most votes. Years below the public list floor show an editorial empty state (not a 404). `?view=categories` on the year URL permanently redirects to `/categories`.
...
- **Top Categories:** up to five most-voted awards in the same fill-row; heading is **{year} Top Categories**. Category names clamp to one line. Every game tied at #1 rotates in the same tie stack as community boards. Category name → that category board; game name → game page. **{year} Top Categories** and bordered **See All** open `/game-of-the-year/[year]/categories`. Bordered **Make picks** / **My picks** opens the GOTY creator on Categories. If no award has enough votes yet, the section still renders taller and centered: “Not enough votes yet” plus a bordered **Add categories to your list**.
...
**Include category votes** is off by default. For **2025**, votes use the
reception-demo per-award pools (same weights as the community 2025 demo —
Best Combat favors Hades II, not Clair Obscur). For **2026**, votes use a
separate per-award pool of titles already out that year (Best Combat favors
Onimusha: Way of the Sword, not Resident Evil Requiem). Other years pick from each
list’s ranked GOTY games (top-rank weighted). **Rewrite category votes only**
keeps existing GOTY lists and replaces those votes.
...
- When **Include category votes** is on: upserts the award catalog (`ensureAwardCategories`) before writing votes; errors if no active categories remain. **2025** writes only the reception-demo awards that match the catalog (Story, Combat, and the rest of that slate), sampled with the same taste profiles as community demo seed — not the GOTY shortlist. Category-only titles must be catalog year 2025. **2026** writes a separate per-award pool of titles already out that year (same ten awards; Best Combat favors Onimusha, not Requiem). Category-only titles must be catalog year 2026, except early-access keys that may still be stored as an earlier IGDB year (Valheim, Slay the Spire 2). Other years reuse the GOTY shortlist with **top-rank weight** (shared with non-demo community edition seed). Errors if 2025 or 2026 category seed finds no catalog matches for those awards.
- **Rewrite category votes only** keeps existing GOTY lists, skips the year game pool, and replaces category votes (and category contrib) for voters `1…N` that already have a list. Does not create voters or lists. 2025 and 2026 still use the per-award pools; other years pick from the list’s current GOTY ranks.
