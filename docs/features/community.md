...
Freeze board writes are **chunked** for Neon HTTP. If a freeze fails mid-write, tables are cleared so the next ensure/rebuild can retry a complete snapshot. Rebuild no longer photocopies every voter’s category picks (that was the multi-minute / timeout path at a few hundred ballots).
