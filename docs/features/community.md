...
Board tallies are computed with **SQL `GROUP BY`** on ballot tables (same `pointsForRank` / plurality rules as before), then chunked into freeze rows for Neon HTTP. Hosts boards join `community_edition_voices`. If a freeze fails mid-write, tables are cleared so the next ensure/rebuild can retry a complete snapshot. Rebuild no longer photocopies every voter’s category picks (that was the multi-minute / timeout path at a few hundred ballots).
