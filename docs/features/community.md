...
- `/communities/[slug]` — home: members see about, leave, and up to three events (hosts with none yet see **Create event**). **Public** non-members see limited about + Join. **Public + joins closed** guests see the member-style overview (events, about) with no Join. **Private** non-members see an invite-only notice.
