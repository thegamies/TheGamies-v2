...
Freeze writes (GOTY, category tallies, voter ranks, voter category picks) are **chunked** for Neon HTTP. If a freeze fails mid-write, tables are cleared so the next ensure/rebuild can retry a complete snapshot (avoids meta-only freezes that hide missing categories).
