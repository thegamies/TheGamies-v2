      ...
      <main className="mx-auto w-full max-w-[var(--page-max)] px-[var(--gutter)] py-10">
        ...
        {error ? (
          <p className="mt-8 text-accent">
            Could not load games. Is <code>DATABASE_URL</code> set and migrated?{" "}
            {error}
          </p>
        ) : games.length === 0 ? (
          <p className="mt-8 text-muted">
            No games yet. Run sync from{" "}
            <Link href="/admin/sync" className="text-ink underline">
              Admin Sync
            </Link>{" "}
            or <code className="text-ink">pnpm sync:igdb:secrets import --year 2026</code>.
          </p>
        ) : (
